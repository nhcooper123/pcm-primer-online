...
In the [Introduction to Comparative Data](https://nhcooper123.github.io/pcm-primer/foundations.html) section of our book, we took some data on chameleons and manipulated/wrangled it so it was ready for analyses. In this exercise we will show you how we did this in R. We use functions from a collection of packages called the [tidyverse](https://www.tidyverse.org/) - these are amazing for data manipulation etc. It's beyond our scope to cover these in detail, but we will show you what we did to get the tidied dataset in the book.
...
## Practical exercise
Using the `frog-eyes-raw-data.csv` dataset and R, create a new dataset that contains only the columns `genus_species`, `Family`, and two new columns containing the *species mean values* for `rootmass` and `eyemean`. We will talk more about this dataset in later exercises, but it comes from Thomas et al. 2020 @thomas2020eye.