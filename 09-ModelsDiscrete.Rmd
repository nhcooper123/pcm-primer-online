...
As an example, let's look at the evolution of habit in our snake data. Before we start let's make sure we have an idea of what these data look like.

```{r, message = FALSE}
# How many species are in each category of Habit?
mydata %>%
  group_by(Habit) %>%
  summarise(number = n())
```

We can also visualize these variables on our tree by plotting them with colours. We need to ensure the data and the tree tip labels are in the same order. We already did this using `make.treedata` but just be sure we can use: 
...
We'll then create a vector of five colours for the Habits. These need to be in alphabetical order, so the colours should be in the order Aquatic, Aquatic burrower, Burrower, Semi-aquatic, Terrestrial.

```{r}
# Set up list of colours
habit.colors <- c("darkblue", "plum", "chocolate", "lightblue", "lightgreen")
```

Finally we can plot the tree with coloured tip labels to match the habits. I've rearranged the colour order in the legend so it makes more sense biologically (i.e. putting the aquatic and Semi-aquatic next to each other rather than ordering it alphabetically). To make things a bit easier to see I've left off the tip labels.

```{r}
# Plot the tree with colours at the tips to represent Habit
plot(mytree, cex = 0.5, adj = c(0.2), type = "fan", 
     no.margin = TRUE, show.tip.label = FALSE)
tiplabels(pch = 16, col = habit.colors[mydata$Habit])
...
Some habits are clustered, but others appear across the phylogeny (this is the result of some pretty cool convergent evolution - check out DEEPAK CITE PAPER for more details).
...
We use the function `pull` to extract just the habit values. We then name these values with the species names from `mydata` using the function `names`. Note that this requires the trait data is in the _same order_ as the tree tip labels, but luckily `make.treedata` does this automatically.

```{r}
# Create habit containing just habit values
habit <- pull(mydata, Habit)
# Make sure habit is a character, not a factor
habit <- as.character(habit)
# Look at the first few rows
head(habit)
# Give log head length names = species names at the tips of the phylogeny
names(habit) <- mydata$tiplabel
# Look at the first few rows
head(habit)
```
...
```{r}
# Fit the ER model
equal <- fitDiscrete(mytree, habit, model = "ER")
```
...
To fit the SYM model we just switch the model name. Note that this might be a bit slow on your computer as we have quite a lot of different habits, so a lot of different rates to fit. A good point to grab a coffee/tea/biscuit!
```{r}
# Fit the SYM model
sym <- fitDiscrete(mytree, habit, model = "SYM")
# Look at the output
sym
```
...
```{r}
# Fit the ARD model
ard <- fitDiscrete(mytree, habit, model = "ARD")
# Look at the output
ard
```
...
The “best” model is the one with the smallest AIC. Here the best model of evolution appears to be the SYM model, whereby rates of transition are the same in both directions, but are different for the different pairs of habits. 
...
To perform ancestral state estimation of Habits for snakes we can use the `habit` object we made earlier and used to fit the ER, SYM and ARD models. Recall that `habit` is just the Habit column from the snake heads dataset, in the same order as the tips in the tree, with names equal to the tip labels/Species names in the tree.

```{r}
# Take a quick look at the first few rows of habit
head(habit)
```

To perform an ancestral state estimation of Habits under the symmetric model (the best fitting model for the data as discovered above) we use this code:
```{r, message = FALSE}
# Ancestral state estimation of Habit under the SYM model
ancestral_habits <- ace(habit, mytree, type = "discrete", model = "SYM")
```
...
```{r, eval = FALSE}
head(ancestral_habits$lik.anc)
```

In this matrix the rows correspond to nodes in the tree (although the numbering is bit off; we don't need to worry about this here but it will come up in the next exercise) and the five columns give the *scaled likelihoods* that the node is in each of the different Habits. 
...
```{r}
# Plot the tree with colours at the tips and nodes to represent Habit
plot(mytree, cex = 0.5, adj = c(0.2), type = "fan", 
     no.margin = TRUE, show.tip.label = FALSE)
tiplabels(pch = 16, col = habit.colors[mydata$Habit])
nodelabels(pie = ancestral_habits$lik.anc, piecol = habit.colors, cex = 0.5)
legend("bottomleft", pch = 15, bty = "n",
       legend = c("Aquatic", "Semi-aquatic", "Aquatic burrower", 
                  "Burrower", "Terrestrial"), 
       col = c("darkblue", "lightblue", "plum", 
...
Aquatic is most commonly estimated as the ancestral Habit for natricine snakes, with Semi-aquatic as a close second, and with multiple transitions to other Habits throughout the tree. 
