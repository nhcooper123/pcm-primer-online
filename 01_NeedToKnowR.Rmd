...
```{block, type = "info"}
Note that many things in R can be done in multiple ways. You should choose the methods you feel most comfortable with. Don't panic if we do it differently!
```
...
Some datasets are built into R and/or various R packages (see below), so you may see us access these just using code like this (`iris` is a dataset built into R):
```{r}
data(iris)
```
...
You'll notice at the start of each exercise we remind you of the packages you need to install and load. To run any specialised analysis in R, you need to download one or more additional packages from the basic R installation. __You need to be connected to the internt to do this__. 
...
If you get other kinds of errors don't panic. Check you are connected to the internet. Read the error message carefully to see if it gives you any clues. If that fails, try pasting it into Google and seeing if anyone else has had the same error.
```
...
R can read files in lots of formats, but for simplicity we'll use comma-delimited or `.csv` files in these exercises. Excel (and many other applications) can output files in this format (it's an option in the `Save As` dialog box under the `File` menu). 
