...
Next check that the species names match up in the tree and the data. This shoul reveal any typos and/or taxonomic differences that need to be fixed before moving any further
...
Let's estimate $\lambda$ for log gestation length.
...
We can use the function `pull` to extract just the GestationLen_d values, and we can log transform all these numbers using `log` if we want to work with log GestationLen_d values.

```{r}
# Create logGL containing just log gestation length values
logGL <- log(pull(mydata, GestationLen_d))
# Look at it
logGL
```
...
```{r}
# Give log GL names = species names
names(logGL) <- mydata$Binomial
# Look at it
logGL
```
...
```{r}
lambdaGL <- phylosig(mytree, logGL, method = "lambda", test = TRUE)
```
`test = TRUE` specifies that we want to run a likelihood ratio test to determine if lambda is significantly different from 0.

To look at the output we just type in the name of the model; `lambdaGL` in this case
```{r}
lambdaGL
```
logL	
...
The $\lambda$ estimate for log GestationLen_d is 0.948. `logL` is the log-likelihood, `logL0` is 	the log-likelihood for lambda of 0, and `P` is the p value from a likelihood ratio test testing whether lambda is significantly different from 0 (no phylogenetic signal).

Here p < 0.001. We interpret this as $\lambda$ being significantly different from 0, i.e. there is significant phylogenetic signal in log gestation length.

### Blomberg's *K* (Blomberg et al 2003)
To estimate Blomberg’s *K* we also use `phylosig` but with `method = K`.

```{r}
KGL <- phylosig(mytree, logGL, method = "K", test = TRUE, nsim = 1000)
```
...
KGL
```
*K* for log gestation length is 0.776. The p value tells us how many times out of 1000, a randomised value of *K* is more extreme than the observed value. If this number is low, the p value is low (e.g. if 5 out of 1000 randomised values of *K* are more extreme than the observed value p = 5/1000 = 0.005). Here p = 0.001, suggesting that only 1 randomised value of *K* was more extreme than the observed value. 

We interpret this as *K* being significantly different from 0, i.e. there is significant phylogenetic signal in log gestation length.
...
Thirs, always carefully consider what variation in values of $lambda$ and *K* across traits and clades really means. It may not tell you as much about your system as you think it does.
