# Understanding, plotting and manipulating phylogenies in R {#phylogenies}

Using phylogenies in R can feel daunting at first, even if you're already familiar with R. Luckily, lots of nice R packages exist to help us work with them, and fairly new packages like `ggtree` mean we can now make beautiful phylogeny plots without needing to resort to other software. In this chapter I'll introduce you to how R stores phylogenies, and then show you how to plot them and manipulate them before using them in your PCMs.

## Before you start {-}
Open your R Project. Mine is a folder called `PCM-exercises`, and it is named `PCM-exercises.RProj`. Within the `PCM-exercises` folder is a subfolder called `data` where I have stored all the data I downloaded from [here](https://download-directory.github.io?url=https://github.com/nhcooper123/pcm-primer-online/tree/main/data)
You will also need to install the following packages: 

1. `tidyverse` - for reading, manipulating and plotting data [@tidyverse]
2. `ape` - functions for reading, plotting and manipulating phylogenies [@ape]
3. `ggtree` - for plotting trees [@ggtree2017e], [@ggtree2018d], [@ggtree2022a], [@ggtree2022b]
4. `patchwork` - to plot multi-panel plots [@patchwork]
5. `ggimage` - to add images to plots 
6. `phytools` - to add species to phylogenies [@phytools]
...
To load a tree into R you need either the function `read.tree` or `read.nexus` from the package APE. `read.tree` can deal with a number of different types of data (including DNA) whereas `read.nexus` reads NEXUS files. Let's practice with our basic tree. We can read it in using `read.tree`. Remember to load the APE library first or this won't work.
...
What is going on here? The easiest way to understand is to look at the `edge` matrix and the tree with node numbers added to it (see below). For all trees, 1 is the number of the first tip taxon of the phylogeny, 2 is the number of the second taxon and so on until you have each taxon numbered. The numbers then refer to *nodes*, working through the tree from the root forwards.
...
```{block, type = "detail"}
You will often see phylogenies with these little silhouette images on the tips, but where do they come from? Generally people get them from [PhyloPic](https://www.phylopic.org/). You can search on this website for the taxa you need, then pick your favourites and save them in an `images/` folder in your project. It's sensible to make sure all animals are facing in the same direction, so you might need to flip some of them first.  

There is also an R package `rphylopic` which can help with adding silhouettes from PhyloPic automatically.

What if PhyloPic doesn't have the taxon you need? My solution is usually to take an image and then cut it out in [Inkscape](https://en.wikipedia.org/wiki/Inkscape), a free tool for editing vector graphics. If you own a license for PhotoShop or similar you could also use this. Help the community by adding any silhouettes you produce to PhyloPic!
```

```{block, type = "warning"}
If you use images from PhyloPic remember to check the license and credit info, and add this to your manuscript - either in the Acknowledgements or in the figure caption. Before getting too excited about your tree make sure that the images are licensed correctly for your intended output, for example, some cannot be used for commercial purposes.
```

```{block, type = "info"}
Credits for the PhyloPics used in these examples
- Deer: Oscar Sanisidro, CC0 1.0.
- Robin: Anthony Caravaggi, CC0 1.0.
- Spider: Melissa Broussard, Attribution 3.0 Unported
- Jellyfish: Michelle Site, Attribution-NonCommercial 3.0 Unported
```
...
## Practical exercise
Read in the `frog-tree.nex` phylogeny from the folder. This comes from Feng et al. 2017 @feng2017phylogenomics. Then do the following.
...
EXTRA. Use ggtree and the ggtree manual to produce a phylogeny plot of this tree that you could use in a publication OR make the most exotic looking tree you possibly can. Playing around with these packages is the best way to learn.