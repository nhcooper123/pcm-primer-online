...
```{block, type = "info"}
We will be using the evolution of eye size in frogs as an example. The data and modified tree come from @thomas2020eye, and the original tree comes from @feng2017phylogenomics. I've removed a few species and a few variables to make things a bit more straightforward. If you want to see the full results check out Thomas et al 2020 @thomas2020eye!
```
...
4. `MCMCglmm` - to fit GLMMs [@MCMCglmm]
5. `coda` - to assess convergence of MCMC runs [@coda]
...
Unfortunately, fitting these models is quite tricky, in part because of the *MCMC* part of the `MCMCglmm` package name. MCMC stands for Markov Chain Monte Carlo, and this is a Bayesian method which means it works somewhat differently to the models we've met before. I'll try to explain as we go along.
...
`MCMCglmm` requires us to set prior distributions for the fixed and random effects. For the random effects this is actually a bit trickier than many help tutorials might suggest. If you're fitting any model with any degree of complexity you'll need to read up on this to get the most appropriate priors.
...
I also recommend reading the following if you want to know more: 
