# Macroevolutionary models for discrete traits {#discrete}
...
4. `phytools` - for plots of transition rates [@phytools]
...
For more details on these models please see [here](https://nhcooper123.github.io/pcm-primer/foundations2.html#modelling-discrete-traits---simple-models)

We are going to use the package `geiger` to fit models in this exercise, so we need the additional preparation step we first met in the chapter on Phylogenetic Signal (see \@ref(signal)). We need to create an object in R that only contains the variable required, and the species names (so we can match it up to the tree). 
