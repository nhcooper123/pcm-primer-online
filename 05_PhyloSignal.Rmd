...
xx <- c(2,4,11)
names(xx) <- c("A", "B", "C")
results <- ace(xx, tree, method = "pic")
root <- results$ace[[1]]
MSE_0 <- sum((xx - root)^2) / (length(xx) - 1)

xxdata <- data.frame(trait = xx, species = c("A", "B", "C"))
comp.data <- comparative.data(phy = tree, data = xxdata, 
                              names.col = species, vcv = TRUE, 
                              na.omit = FALSE, warn.dropped = TRUE)
model <- pgls(trait ~ 1, 
                   data = comp.data, lambda = 1)
model$Vt

phylosig(tree, xx, method = "K", test = TRUE, nsim = 1000)
```
