...
```{block = "info"}
We will be using the diversification rates of dragonflies (Anisoptera) as an example. The tree comes from @letsch2016not and is available to download from @letsch2016data. I'm only using one of their trees. If you want to see the full results check out Letsch et al. 2016 @letsch2016not!
```
...
1. `ape` - functions for reading, plotting and manipulating phylogenies [@ape]
2. `BAMMtools` - functions to work with outputs from BAMM [@BAMMtools]
3. `coda` - to assess convergence of MCMC runs [@coda]
4. `phytools` - for lineage through time plots and force.ultrametric [@phytools]
5. `viridis` - colour palettes [@viridis]
...
```
It's clear from this plot that the diversification of dragonflies is close to the expectation under a pure-birth model, but doesn't totally match it.
...
```
This gives us a vague idea of whether your run has converged. SPOILER alert, it has because I played around with the parameters in BAMM until it did, and used the same priors as Letsch et al. 2016 @letsch2016not. 
