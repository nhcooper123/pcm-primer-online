...
- rooting
- removing species
- adding species [needs to be added - code is in office]

## Before you start
- Open the `03_Phylogenies.RProj` file in the `03_Phylogenies` folder to open your R Project for this exercise.
...
* A set of __aesthetic mappings__. These describe how variables in the data are associated with the aesthetic properties of the layer. This can include what to use as x and y axes, and the colour and size of the objects (e.g. points) on a plot. Each layer can be associated with its own unique aesthetic mappings. Aesthetic mappings are alwasy defined inside the `aes` funtion. Different geoms will need different sets of aesthetic mappings, for example to draw a scatter plot you need to define the x and y axes, but for a histogram you only need the x axis.. 
...
Another thing I'm going to do below with `ggtree` is plot multiple plots on one plotting space. There are a couple of ways to do this, but the easiest is to use the package `patchwork`. First we need to assign our plots to an object using `<-`. So for the example above...
...
With `patchwork`, we can use `+` to plot things next to each other, or `/` to plot things on top of one another, and `()` to do more complex arrangements. See https://patchwork.data-imaginist.com/articles/patchwork.html for details.
...
```
```{r}
myplot / (myplot + myplot)
```

Below you'll see I use the `ggplot2` layer `labs` to add A, B, C etc labels to plots in multi-panel figures
...
To plot the basic phylogeny we've been practicing with we use:
...
ggtree(tree) + 
  # Add theme for trees
  theme_tree() +
  # Change x and y limits so labels fit
  xlim(0, 22) +
  ylim(0, 5) +
...
This is clearly a bit more involved than just using `plot.phylo` with `ape`. If you're happy with plotting in `ape` you don't need to use `ggtree`. As I mentioned above, for day-to-day tree viewing and checking I just tend to use `ape`, so don't worry if this is all incomprehensible.

One thing, among many, that `ggtree` is great for is adding images to the tips rather than species names. To do this you need to collate the images you want to use first. You may want to draw your own, or I often download them from the excellent [PhyloPic](http://phylopic.org/) website (these are free to use but don't forget to record who made the image so you can credit them in your papers etc.). To keep my folder tidy I keep images in a separate subfolder called, predictably, `images`. You need the package `ggimage` installed for this to work.
...
Note that to add eitehr text or images to the tips we use the same layer function, `geom_tiplab`. If doing this with your own data you'll likely need to fiddle with the sizes of the images to get them looking right. 

Some of you may be wondering what the `rep(NA, 3)` bit of the code above does. When we try to add images to the tips, the function also tries to add images to the internal nodes. We don't want these to have images so we just ask `ggtree` to use `NA`, i.e. don't add an image. `rep(NA, 3)` means replicate `NA` three times (for the three internal nodes). Again you'll probably need to increase the number when you do this for your own data.

As with `plot.phylo` we can plot phylogenies in different ways, see code below. Fan trees are called circular trees. Note the different values for offsets and hjust (horizontal justification) I've used here to make the labels fit. Again you will need to fiddle with these numbers to get this to work for your data.
...
There are many more things you can do with `ggtree`, all based on this idea of adding layers. See the [manual for `ggtree`](https://yulab-smu.github.io/treedata-book/index.html) for more details.
...
plot(bird.orders)
```
...however, if you're looking to do something more complex like removing entire clades or functional groups it's probably easier to use the `treeplyr` functions in the next exercise.

#### Adding species

BIND TREE - code is on computer in work...
...
For the next two exercises we are going to use XXXXX phylogeny. Download it (or any other phylogeny you have to hand) and see if you can do the following:
