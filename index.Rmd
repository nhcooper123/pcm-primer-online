...
2. Open the .Rmd (R Markdown) file for each exercise in RStudio and use it as an RNotebook. This allows you to run chunks of code in the script and for the results to appear below the code. You need to open the .Rmd file in the same folder as the one with the `data/` folder for that exercise. You can then just click the little green play buttons to the far right of each code chunk (grey boxes with R code in them) to run the code. The outputs, graphs and results will all appear in the .Rmd file underneath the code. This is a good way to learn/teach these materials. See [here](http://rmarkdown.rstudio.com/r_notebooks.html) for more details.
3. All the code and outputs are shown in this book, so you can also use this as a reference and then use the code to complete the practical exercises at the end of each chapter, or apply them to your own data.
...
Snakes are also cool, especially natricines which are the group that contains both the delightful European grass snake (*Natrix natrix*) and the ubiquitous garter snakes (genus *Thamnophis*) of North America. Natricine snakes are found across the globe, and have a range of interesting ecologies and more morphological variation than you might expect, especially in their head shape. Deepak [CITE PAPER] predicted that these variations in head shape would be more closer related to the ecomorph they belonged to (i.e. whether the snake was terrestrial, aquatic, burrowing or aquatic burrowing) than their evolutionary history. They expected that head shape might be an example of convergent evolution. In our examples we'll test some of these hypotheses using phylogenetic comparative methods.

The data for this example comes from DEEPAK [CITE PAPER], and the tree comes from DEEPAK [CITE PAPER]. If you want to see the full results check out DEEPAK [CITE PAPER]! And there's a nice summary of the paper [here]().

### Diversification in marsupial mammals {-}
You've probably guessed that yes, marsupial mammals are also cool. They include kangaroos and wallabies, koalas, wombats, bilbies, potoroos, possums, opossums, Tasmanian devils, quolls, dunnarts, numbats, marsupial moles and the tiny monito del monte, sole member of the order Microbiotheria. But how quickly did they diversify? And do different clades have different rates of evolution? In our examples we'll test some of these questions using phylogenetic comparative methods.

The tree for this example comes from @bininda2007delayed. This paper looked across all mammals to see how they diversified after the K-Pg extinction event. If you want to see the full results for all mammals check out @bininda2007delayed! 
